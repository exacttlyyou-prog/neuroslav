...
# Таймаут обработки webhook: Telegram требует ответ < 60 сек
WEBHOOK_PROCESS_TIMEOUT = 55


@router.post("/webhook")
async def telegram_webhook(update: SecureTelegramUpdate):
    """Обрабатывает входящие сообщения от Telegram. Без зависимости от БД — при падении БД handler всё равно вызывается и может отправить ответ."""
    update_id = getattr(update, "update_id", None)
    _fallback_chat_id = ""
    if getattr(update, "message", None):
        _fallback_chat_id = str((update.message or {}).get("chat", {}).get("id", ""))
    logger.info("WEBHOOK recv update_id=%s chat_id=%s", update_id, _fallback_chat_id or "-")
    ...
    try:
        ...
        if not telegram_auth.verify_telegram_webhook(chat_id):
            ...
            # Отвечаем пользователю, чтобы он видел причину и мог добавить chat_id в ADMIN_CHAT_ID
            try:
                from app.services.telegram_service import TelegramService
                _tg = TelegramService()
                await _tg.send_message_to_user(
                    chat_id=chat_id,
                    message=f"🔒 <b>Нет доступа.</b>\n\nТвой chat_id: <code>{chat_id}</code>\n\nДобавь его в .env:\n<code>ADMIN_CHAT_ID={chat_id}</code>",
                    parse_mode="HTML"
                )
            except Exception as e:
                logger.debug(f"Не удалось отправить сообщение об отказе: {e}")
            return {"ok": True}  # Возвращаем ok для Telegram, но не обрабатываем
        
        # Сразу отправляем автоответ по системному промпту — до любых тяжёлых операций (DailyCheckinService, process_response и т.п.)
        try:
            from app.services.telegram_service import TelegramService
            _tg = TelegramService()
            await _tg.send_message_to_user(chat_id=chat_id, message=get_neural_slav_thinking_response())
        except Exception as early_err:
            logger.warning(f"Ранний автоответ не отправлен: {early_err}")
        ...
        if is_forwarded:
            try:
                ...
                # Автоответ уже отправлен в начале обработки сообщения
                
                from app.services.agent_router import AgentRouter
                router = AgentRouter()
                classification = await asyncio.wait_for(
                    router.classify(forwarded_context), timeout=WEBHOOK_PROCESS_TIMEOUT - 5
                )
                logger.info(f"📋 Классификация пересылки: {classification.agent_type} (уверенность: {classification.confidence:.2f})")
                agent_response = await asyncio.wait_for(
                    router.route(forwarded_context, classification, sender_username=sender_username, sender_chat_id=chat_id),
                    timeout=WEBHOOK_PROCESS_TIMEOUT - 10
                )
                ...
                # Показываем важные действия (первый агент + цепочка)
                all_actions_fwd = list(agent_response.actions or [])
                for ch in (agent_response.metadata or {}).get("chain_responses", []):
                    all_actions_fwd.extend(ch.get("actions") or [])
                if all_actions_fwd:
                    user_friendly_actions = base_agent.format_user_friendly_actions(all_actions_fwd)
                ...
                logger.error("WEBHOOK timeout при обработке пересылаемого сообщения update_id=%s", update_id)
                try:
                    await service.telegram.send_message_to_user(
                        chat_id=chat_id,
                        message="⏱ Слишком долго. Напиши короче или попробуй позже."
                    )
                except Exception:
                    pass
                return {"ok": True}
                ...
                try:
                    await service.telegram.send_message_to_user(
                        chat_id=chat_id,
                        message=f"❌ Ошибка при обработке пересылаемого сообщения: {str(e)[:150]}"
                    )
                except Exception:
                    pass
                return {"ok": True}
        ...
        # Команды подтверждения встречи (сессия БД только здесь)
        ...
        # Сначала проверяем, является ли это ответом на ежедневный опрос (сессия БД только здесь)
        async with AsyncSessionLocal() as db:
            clarification = await service.process_response(chat_id, text, db)
        
        if clarification:
            # Отправляем уточняющий вопрос (автоответ уже отправлен в начале обработки)
            await service.telegram.send_message_to_user(
                chat_id=chat_id,
            ...
            # Автоответ уже отправлен в начале обработки сообщения — продолжаем AgentRouter
            # Обрабатываем через AgentRouter для свободного текста (таймаут — Telegram требует ответ < 60 сек)
            try:
                from app.services.agent_router import AgentRouter
                router = AgentRouter()
                logger.info(f"🤖 Обработка свободного текста через AgentRouter: {text[:50]}...")
                classification = await asyncio.wait_for(
                    router.classify(text), timeout=WEBHOOK_PROCESS_TIMEOUT - 5
                )
                logger.info(f"📋 Классификация: {classification.agent_type} (уверенность: {classification.confidence:.2f})")
                agent_response = await asyncio.wait_for(
                    router.route(text, classification, sender_username=sender_username, sender_chat_id=chat_id),
                    timeout=WEBHOOK_PROCESS_TIMEOUT - 10
                )
                ...
                # Показываем важные действия пользователю: первый агент + действия из цепочки
                all_actions = list(agent_response.actions or [])
                for ch in (agent_response.metadata or {}).get("chain_responses", []):
                    all_actions.extend(ch.get("actions") or [])
                if all_actions:
                    user_friendly_actions = base_agent.format_user_friendly_actions(all_actions)
                    if user_friendly_actions:
                        actions_text = "\n".join(user_friendly_actions)
                        actions_text = sanitize_html_for_telegram(actions_text)
                        await service.telegram.send_message_to_user(
                ...
                logger.error("WEBHOOK timeout при обработке свободного текста update_id=%s", update_id)
                try:
                    await service.telegram.send_message_to_user(
                        chat_id=chat_id,
                        message="⏱ Слишком долго. Напиши короче или попробуй позже."
                ...
                try:
                    from app.services.telegram_service import sanitize_html_for_telegram
                    error_message = sanitize_html_for_telegram(f"❌ Ошибка при обработке: {str(e)[:150]}")
                    await service.telegram.send_message_to_user(chat_id=chat_id, message=error_message)
                except Exception as send_error:
                    logger.error(f"❌ Критическая ошибка при отправке сообщения об ошибке: {send_error}")
        
        return {"ok": True}
    except Exception as e:
        logger.error("WEBHOOK fail update_id=%s err=%s", update_id, str(e), exc_info=True)
        if _fallback_chat_id:
            try:
                from app.services.telegram_service import TelegramService
                await TelegramService().send_message_to_user(
                    chat_id=_fallback_chat_id,
                    message="⚠️ Что-то пошло не так. Повтори позже или напиши короче."
                )
            except Exception:
                pass
        return {"ok": True}
