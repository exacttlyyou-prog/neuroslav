...
class RAGService:
    ...
    async def sync_with_notion(self, notion_service, parent_page_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            parent_id = parent_page_id or getattr(self.settings, "notion_ai_context_page_id", None) or self.settings.notion_meeting_page_id
            if not parent_id:
                logger.warning("Не указан parent_page_id и (NOTION_AI_CONTEXT_PAGE_ID или NOTION_MEETING_PAGE_ID) для синхронизации")
