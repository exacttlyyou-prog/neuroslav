...
"""
    ...
    async def route(self, user_input: str, classification: IntentClassification, sender_username: str = None, sender_chat_id: str = None) -> AgentResponse:
        """
        ...
        Args:
            ...
            sender_chat_id: Telegram chat_id отправителя — куда слать отложенные сообщения (опционально)
        ...
        try:
            ...
            if agent_type == "task":
                ...
                # default / неясный intent — полный pipeline «7 шагов» (3–7)
                from app.services.pipeline_orchestrator import SevenStepPipeline
                pipeline = SevenStepPipeline()
                response = await pipeline.run(
                    user_input,
                    sender_chat_id=sender_chat_id,
                    sender_username=sender_username
                )
                decision_trace["selected_agent"] = "pipeline"
                decision_trace["reasoning"] = (
                    classification.reasoning or "Неясное намерение — полный pipeline (интерпретация → RAG → саммари → ToV → получатели)"
                )
            
            if agent is not None:
                response = await agent.process(user_input, classification, sender_username=sender_username, sender_chat_id=sender_chat_id)
            
            # Цепочка агентов только для классических агентов (у pipeline своей цепочки нет)
            next_agents = []
            if agent is not None:
            ...
            if next_agents:
                ...
                chain_responses = await self.coordinate_agents(
                    response, user_input, next_agents,
                    sender_username=sender_username, sender_chat_id=sender_chat_id
                )
            ...
            # Для pipeline сохраняем его pipeline_steps из метаданных
            if getattr(response, "agent_type", None) == "pipeline" and response.metadata:
                pt = (response.metadata.get("decision_trace") or {})
                decision_trace.setdefault("pipeline_steps", pt.get("pipeline_steps"))
    ...
    async def coordinate_agents(
        ...
        Args:
            ...
            sender_username: Telegram username отправителя (проброс в агентов цепочки)
            sender_chat_id: Telegram chat_id отправителя (проброс в агентов цепочки)
        ...
        for agent_type in next_agent_types:
            try:
                ...
                # Обрабатываем через агента (проброс sender_* для отложенных сообщений и т.п.)
