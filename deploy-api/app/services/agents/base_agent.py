...
class BaseAgent(ABC):
    ...
    async def process(self, user_input: str, classification: IntentClassification, sender_username: str = None, sender_chat_id: str = None) -> AgentResponse:
        """
        ...
        Args:
            ...
            sender_chat_id: Telegram chat_id отправителя — куда слать отложенные сообщения/напоминания (опционально)
        ...
        try:
            ...
            # Шаг 2: Обработать через конкретный агент с передачей username и chat_id
            logger.info(f"Обработка через {self.__class__.__name__}...")
            result = await self._process_with_context(user_input, classification, context, sender_username=sender_username, sender_chat_id=sender_chat_id)
    ...
    def format_user_friendly_actions(self, actions: List[Dict[str, Any]]) -> List[str]:
        ...
        action_map = {
            ...
            "tasks_created_from_meeting": "📋 Задачи созданы из встречи",
